...
def make_xhttp_link(uuid: str, host: str, mode: str, label: str, port: int = 443, fp: str = "chrome", alpn: str = "h2,http/1.1", remark_override=None) -> str:
    path = f"/xhttp-siz10/{mode}/{uuid}"
    params = {"encryption": "none", "security": "tls", "type": "xhttp", "mode": mode, "host": host, "path": path, "sni": host}
    if fp:
        params["fp"] = fp
    if alpn:
        params["alpn"] = alpn
...
# aliasها: XHTTP داخل خود server.py ادغام شده، پس مستقیم به گلوبال‌های همین فایل وصل می‌شن
LINKS = CONFIGS
LINKS_LOCK = LOCK
stats = STATS
connections = CONNS
logger = log
is_link_allowed = is_allowed
check_and_use = count_use
parse_vless_header = parse_vless
